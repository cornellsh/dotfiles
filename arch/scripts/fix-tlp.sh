#!/bin/bash
# Fix TLP configuration (run as root)

echo "Fixing TLP configuration..."

# Update the TLP config file with correct settings
cat > /etc/tlp.d/99-battery-optimization.conf <<'EOF'
# TLP battery optimization configuration for ASUS Zenbook UX5401ZA
...
# Stop docker socket if still running
...
echo ""
echo "Run 'tlp-stat -p' to verify CPU settings"
echo "Run 'tlp power-saver' to activate power-saver mode"
